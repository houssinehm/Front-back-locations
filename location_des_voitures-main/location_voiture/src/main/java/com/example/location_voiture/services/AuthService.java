...
public class AuthService {
    ...
    public ApiResponse<String> register(RegisterRequest request) throws Exception {
        ...
        if (userRoles.contains(roleRepository.findByName(RoleName.ROLE_CLIENT).orElseThrow())) {
            ...
            client.setFirst_name(request.getFirst_name()); // <-- bien récupérer les données
        ...
        if (userRoles.contains(roleRepository.findByName(RoleName.ROLE_AGENCE).orElseThrow())) {
            Agency agency = new Agency();
            agency.setNom_agence(request.getNom_agence());
            agency.setNombre_voiture(request.getNombre_voiture());
            agency.setAdresse(request.getAdresse());
            agency.setUtilisateur(utilisateur); // associer utilisateur à l'agence
            agencyRepository.save(agency);
        }

        // Générer le lien d'activation
        String activationLink = "http://localhost:4200/auth/activation?token=" + utilisateur.getActivationToken();
